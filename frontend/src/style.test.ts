...
describe('Donote shell styles', () => {
  ...
  test('defines focused command workspace styling hooks for the main surfaces', () => {
    ...
    expect(cssBlock('.command-toolbar')).toMatch(
      /grid-template-columns:\s*minmax\(0,\s*0\.9fr\) minmax\(0,\s*1\.1fr\) auto/,
    )
    expect(cssBlock('.command-status')).not.toBe('')
    expect(cssBlock('.command-count')).not.toBe('')
    expect(cssBlock('.workspace-layout')).toMatch(/display:\s*contents/)
    expect(cssBlock('.workspace-sidebar')).toMatch(/background:\s*color-mix\(in srgb,\s*var\(--surface-muted\)/)
    expect(cssBlock('.workspace-sidebar')).toMatch(/grid-row:\s*2 \/ 4/)
    expect(cssBlock('.sidebar-header')).toMatch(/padding:\s*12px/)
    expect(cssBlock('.workspace-title')).toMatch(/grid-template-columns:\s*30px minmax\(0,\s*1fr\) auto/)
    expect(cssBlock('.workspace-action')).toMatch(/width:\s*28px/)
  ...
  test('keeps command buttons square and accent-backed when active', () => {
    ...
    expect(toolbarBlock).toMatch(/min-height:\s*54px/)
    expect(toolbarBlock).toMatch(/gap:\s*12px/)
    expect(toolbarBlock).toMatch(/padding:\s*8px 14px/)
    expect(cssBlock('.command-brand')).toMatch(/display:\s*flex\b/)
    expect(cssBlock('.brand-mark')).toMatch(/width:\s*30px/)
  ...
  test('keeps custom document tabs compact without legacy tab selectors', () => {
    ...
    expect(tabsBlock).toMatch(/min-height:\s*42px/)
    expect(tabsBlock).toMatch(/background:\s*color-mix\(in srgb,\s*var\(--surface-muted\)/)
    expect(trackBlock).toMatch(/height:\s*42px/)
    expect(trackBlock).toMatch(/padding:\s*7px 24px 6px/)
    expect(listBlock).toMatch(/display:\s*flex\b/)
    expect(tabItemBlock).toMatch(/max-width:\s*210px/)
    expect(tabItemBlock).toMatch(/height:\s*29px/)
    expect(tabItemBlock).toMatch(/margin-right:\s*6px/)
    expect(tabItemBlock).toMatch(/border-radius:\s*8px/)
    ...
    expect(activeTabBlock).toMatch(/box-shadow:\s*var\(--shadow-control\)/)
    expect(tabLabelBlock).toMatch(/gap:\s*5px/)
    expect(closeButtonBlock).toMatch(/width:\s*24px/)
    expect(closeButtonBlock).toMatch(/height:\s*24px/)
  ...
  test('keeps search panel controls on native Donote classes', () => {
    ...
    expect(panelBlock).toMatch(/min-height:\s*48px/)
